...
export function resolveSettings(config = {}) {
  return {
    ...
    // On by default, and the default is the whole point of the field: a tool
    // the model never thinks to call is a tool that does not exist, and the
    // measured failure this plugin had was exactly that — defined, registered,
    // and never chosen. Off is for a deployment that wants the tools available
    // without the standing line.
    announceCapability: config.announceCapability !== false,
  };
}
