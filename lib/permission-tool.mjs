 ...
 * the user guessing; an agent that can report "capture failed, here is the entry
 * to switch on, and here is the pane" gets unblocked in one exchange.
 *
 * The entry is named rather than the executable path, because macOS names the
 * list entry after the app the request is attributed to — the app that started
 * the harness — so the name depends on how DSH was launched and cannot be
 * computed from inside the process. The tool states the principle and lists the
 * common cases; `lib/permission.mjs` has the measurement behind that.
...
export function screenPermissionTool(permission, readSettings, issues = []) {
  return defineTool({
    ...
    description: [
      ...
      'Call it with action "check" to diagnose a refused screenshot, or with action "guide" to walk the user through fixing it — it checks first, opens the System Settings pane only when the grant really is missing, and names the entry to switch on.',
    ...
    parameters: {
      action: {
        ...
        description:
          ...
          + 'and names the entry to look for. When the grant is already in place "guide" says so and opens nothing.',
      },
    },
    output: {
      schema: {
