 ...
 *
 * This is the *provider's* limit, not a policy of this plugin's. The DeepSeek
 * adapter caps one request at 600 images — `maxImagesPerRequest`, default 600 —
 * and beyond it the harness substitutes the excess with a text placeholder
 * rather than failing, so a burst larger than this would come back as frames
 * the model cannot see. Refusing at the real ceiling is the honest version;
 * refusing earlier was this plugin's own guess, and the guess was wrong.
 *
 * What actually bounds a sensible burst is the context, not a count: one image
 * costs at most 384 vision tokens (measured at about 380 for a full-screen
 * capture), so the frames are the model's to price. Ten frames is about 4k
 * tokens and a hundred is about 38k, which is a decision the caller is better
 * placed to make than the plugin is — it can see the conversation this will
 * land in and the plugin cannot. The count is offered as headroom and the reply
 * reports what it cost.
 */
export const MAX_BURST_FRAMES = 600;

/**
 * Vision tokens one frame can cost, at most.
 *
 * The adapter's own accounting prices a request image at no more than this, so
 * `frames x this` is the upper bound on what a burst adds to the conversation.
 * Published here so the tool description can state the arithmetic rather than
 * describe it: a model choosing between ten frames and sixty is choosing
 * between about 4k tokens and about 23k, and it should be able to.
 */
export const MAX_TOKENS_PER_FRAME = 384;

/**
 * Frames a self-terminating burst asks for when the caller named only an
 * interval.
 *
 * Not the ceiling: this is the headroom a burst carries when the picture
 * decides its ending, sized so that any plausible one-shot process fits inside
 * it. Sixty samples is twelve seconds at a 200ms interval and sixty at a
 * second apart, which is the point — the caller chooses the sampling rate for
 * the timescale it is watching, and this covers sixty of them whatever that
 * rate is.
 *
 * It is deliberately far below `MAX_BURST_FRAMES`, because the cap matters in
 * exactly one case: a picture that never settles — a loop, a video, a spinner.
 * There the ending never comes, the headroom is what gets taken, and sixty
 * frames is about 23k tokens while six hundred would be a conversation-ending
 * 230k. A caller that genuinely wants more samples of something endless can say
 * so: `frames` remains open up to the provider's own limit.
 */
export const STILL_HEADROOM_FRAMES = 60;
...
export function planCapture(args, options = {}) {
  ...
  if (wantedFrames !== undefined && wantedFrames > MAX_BURST_FRAMES) {
    throw new CaptureError(
      `frames is ${wantedFrames}, above the ${MAX_BURST_FRAMES} one request can carry: the DeepSeek adapter caps a `
      + 'request at that many images and replaces the rest with a placeholder, so the extra frames would be taken '
      + 'and then not seen. Take fewer frames in this call and repeat it, or raise interval_ms and cover more time '
      + 'with fewer pictures.',
    );
  }
  ...
  // A burst that ends when the picture settles spends its frame cap as
  // headroom, not as a plan: the frames actually taken are however many the
  // motion spans, and the cap only decides what happens if the motion never
  // stops. So when the caller gave an interval but no frame count of their own
  // — the shape of "record this transition at this spacing" — the ceiling is
  // raised well above the ordinary default. A short transition ends the burst
  // early anyway, so the headroom costs nothing there, and it is the difference
  // between catching a 700ms animation and missing its second half. A caller
  // who named a frame count or a window has already answered this and is left
  // alone.
  const untilStill = endsWhenStill(args, waitForChange, frames);
  if (untilStill && wantedFrames === undefined && wantedDuration === undefined) {
    frames = STILL_HEADROOM_FRAMES;
  }
  ...
  return {
    ...
    // The thresholds travel in the plan rather than being imported by the
    // platform: a platform module cannot reach back into this one without
    // forming a cycle, and two copies of "how much movement counts" is exactly
    // the kind of thing that drifts apart.
    ...(untilStill
      ? { untilStill: true, stillFraction: CHANGE_FRACTION, stillConfirmations: STILL_CONFIRMATIONS }
      : {}),
  };
}

/**
 * Whether a burst should end when the picture settles.
 *
 * Asked for, always: a caller who names `until_still` knows what it wants, and
 * wanting it without a wait first is a real job — watching a process until it
 * finishes rather than watching a transition start.
 *
 * Implied whenever the call waited for a change and takes more than one frame.
 * There the frames are a recording of one transition, and the transition ending
 * is the only honest place to stop: the frame count cannot find it, since eight
 * frames of a 300ms transition are three frames of motion and five of a stopped
 * screen, while three frames of a 900ms one miss two thirds of it. That is not
 * a preference to be configured, it is what the call was for, so it is not left
 * for the caller to remember.
 *
 * A single frame after a change is a photograph of the new state and has
 * nothing to end early; a burst nobody waited for was asked for by window or by
 * frame count, and there the caller has already said how long to look.
 * `until_still: false` is how a caller who did wait says they meant that span
 * instead.
 *
 * @param args - the raw tool arguments.
 * @param waitForChange - whether the plan waits for the picture to move.
 * @param frames - how many frames the plan would take at most.
 * @returns whether the burst should end on two consecutive still frames.
 */
function endsWhenStill(args, waitForChange, frames) {
  if (args.until_still === true) return true;
  if (args.until_still === false) return false;
  return waitForChange === true && frames > 1;
}

/**
 * How many consecutive still frames end a burst early.
 *
 * This is the answer to "when is the animation over?", which the frame count
 * cannot answer: a caller who asks for eight frames of a 300ms transition gets
 * three that show it and five that show a screen that has stopped, and a caller
 * who asks for three of a 900ms one gets a third of it. Asking the *screen* is
 * the only way to get both — so when `until_still` is set, the burst keeps
 * looking at what it is capturing and stops once the picture settles.
 *
 * Two, not one: a transition can hold still in the middle — an eased step, a
 * pause between two phases — and a single still frame would end the burst right
 * there. One more frame costs one interval and buys the whole tail of the
 * animation.
 */
export const STILL_CONFIRMATIONS = 2;

/**
 * How long a burst waits for the picture to change before giving up.
 *
 * The wait has to cover everything between the call and the animation: the user
 * has to be told that something is watching, read that, and then trigger it.
 * Ten seconds looked generous and is not — a person reading a message and moving
 * a hand takes longer — so the default is thirty, which is still well inside the
 * tool's own budget and still ends with an error rather than a silence when
 * nobody triggers anything.
 */
...
export async function captureFrames(plan, options) {
  ...
  if (plan.waitForChange !== undefined) {
    // The wait lives inside the call's budget rather than beside it: a wait
    // allowed to outlast the budget would spend the capture's time before the
    // capture started, and the caller would get a timeout with nothing in it.
    // Asking to wait longer than the call may take is answered with the call's
    // own limit, which is the honest reading of the two numbers together.
    const waitMs = Math.min(plan.waitForChange.timeoutMs, budgetMs);
    const seen = await sleepUntilChange(
      waitMs === plan.waitForChange.timeoutMs ? plan : { ...plan, waitForChange: { timeoutMs: waitMs } },
      options,
    );
    if (!seen) {
      throw new CaptureError(
        `nothing on screen changed within ${waitMs}ms, so there was no animation to watch. `
        + (waitMs < plan.waitForChange.timeoutMs
          ? `That is this call's own budget (timeout_ms), not the ${plan.waitForChange.timeoutMs}ms asked for: `
            + 'raise timeout_ms if the wait needs longer. '
          : '')
  ...
  // What is left of the call's budget, not all of it again. Waiting for a user
  // to press a button spends real time, and handing the platform the original
  // budget would let a capture run past the point where the harness gives up on
  // the tool — the frames would be taken and then thrown away. The tools that
  // wait are the ones that need a burst after the wait, so the two draw on one
  // budget rather than each holding a copy of it; see DEFAULT_TIMEOUT_MS in
  // lib/settings.mjs, which is sized for the worst case both halves allow.
  const captureOptions = {
    ...options,
    timeoutMs: Math.max(1000, budgetMs - (Date.now() - issuedAt)),
  };

  if (typeof platform.captureBurst === 'function') {
    const burst = await platform.captureBurst(plan, { ...captureOptions, framePath: at });
    return waited === true ? { ...burst, waitedForChange: true } : burst;
  }
  const frames = [];
  let stillRun = 0;
  let endedBecause = 'frames';
  let previous = null;
  for (let index = 0; index < plan.frames; index += 1) {
    ...
    const captured = await captureScreen(plan, { ...captureOptions, outputPath: at(index) });
    frames.push({ ...captured, startedAt: captureStart, takenAtMs: Date.now() });

    // "Has it stopped?" asked of the frames themselves. A platform that takes a
    // whole burst in one engine call answers this inside that call; a platform
    // driven frame by frame — macOS — answers it here, by comparing the picture
    // it just took with the one before it.
    if (plan.untilStill === true) {
      const current = frameSignature(await readFile(captured.outputPath));
      if (previous !== null && current === previous) {
        stillRun += 1;
        if (stillRun >= STILL_CONFIRMATIONS) {
          endedBecause = 'still';
          break;
        }
      } else {
  ...
  return {
    frames,
    spacingMs: spacing,
    ...(plan.untilStill === true ? { endedBecause } : {}),
    ...(waited === true ? { waitedForChange: true } : {}),
  };
}

/**
 * A comparable signature of one frame's pixels.
 *
 * Used only to answer "has the picture stopped changing?", so it compares the
 * image data with the descriptive chunks removed — the same trick the
 * attachment path uses, and the only way to compare two `screencapture` outputs
 * at all: both carry a timestamp macOS writes into an iTXt record, so the raw
 * bytes of two identical frames differ while the pixels do not.
 *
 * @param data - the frame's bytes.
 * @returns a signature that is equal exactly when the pixels are.
 */
