...
export function planCapture(args, options = {}) {
  ...
  // — the shape of "record this transition at this spacing" — take the cap the
  // tool allows rather than the smaller default. Asking for ten costs nothing
  // when a 300ms transition ends the burst at five, and it is the difference
  // between catching a 700ms one and missing its second half. A caller who
  // named a frame count or a window has already answered this and is left
  // alone.
  const untilStill = endsWhenStill(args, waitForChange, frames);
  if (untilStill && wantedFrames === undefined && wantedDuration === undefined) {
    frames = MAX_BURST_FRAMES;
  }
...
export async function captureFrames(plan, options) {
  ...
  const captureOptions = {
    ...options,
    timeoutMs: Math.max(1000, (options.timeoutMs ?? DEFAULT_TIMEOUT_MS) - (Date.now() - issuedAt)),
  };
