...
/**
 * Where scripts handed to a one-shot process are kept, named after their text.
 *
 * Exported so a test can find and read one, and so the naming rule has a single
 * definition.
 * @param script - the PowerShell source.
 * @returns the absolute path of the cached script for this text.
 */
export function runScriptPath(script) {
  const digest = createHash('sha256').update(script).digest('hex').slice(0, 16);
  return join(tmpdir(), 'dsh-screen-eye-run', `run-${digest}.ps1`);
}

export { runScript as runOneShot };
...
public static int SampleDifference(byte[] current, byte[] previous) {
  if (current == null || previous == null) return -1;
  int limit = Math.Min(current.Length, previous.Length);
  int changed = 0;
  for (int i = 0; i + 2 < limit; i += 3) {
    if (Math.Abs(current[i] - previous[i]) > 8
        || Math.Abs(current[i + 1] - previous[i + 1]) > 8
        || Math.Abs(current[i + 2] - previous[i + 2]) > 8) changed++;
  }
  return changed;
}`;
 ...
 * when somebody did.
 *
 * The fourth argument asks for a sample of the same pixels, which is what lets
 * a burst notice that the animation has stopped. It is taken from the locked
 * bits that were just written rather than from a second look at the screen: a
 * second look would be a look at a *different* screen, one interval later, and
 * would answer a different question.
 *
 * @returns the PowerShell function definition.
 */
function frameFunction() {
  return `function Save-Frame([System.Drawing.Rectangle]$rect, [string]$path, [bool]$drawCursor, [bool]$sample) {
  ...
  try {
    ...
    try {
      $black = [DshScreenEye.Native]::BlackPermille($bits.Scan0, $bits.Stride, $bitmap.Width, $bitmap.Height)
      if ($sample) { $sampleBytes = [DshScreenEye.Native]::Sample($bits.Scan0, $bits.Stride, $bitmap.Width, $bitmap.Height, ${FINGERPRINT_SAMPLE_BUDGET}) }
    }
  ...
  return @{ black = $black; cursor = $cursor; sample = $sampleBytes }
}`;
}
...
try {
  ...
  ${frameFunction()}
  $frame = Save-Frame $rect '${quote(outputPath)}' ([bool]$request.cursor) $false
...
/**
 * The burst loop, as PowerShell — written once for both callers.
 *
 * The loop is the same whether it runs in a one-shot process or inside the
 * resident engine, and the still-detection inside it is the reason this is a
 * shared fragment at all: the frame count cannot answer "is the animation
 * over?", and a rule that answered it differently in two places would be worse
 * than no rule. It reads `$paths`, `$interval`, `$cursor` and `$sample`, which
 * each caller sets before running it.
 *
 * @param untilStill - whether the loop may end early because the picture settled.
 * @returns the PowerShell source.
 */
function burstLoop(plan) {
  const still = plan.untilStill === true
    ? `
    if ($null -ne $frame.sample) {
      if ($null -ne $previousSample) {
        $moved = [DshScreenEye.Native]::SampleDifference($frame.sample, $previousSample)
        $points = [int]($frame.sample.Length / 3)
        if ($points -gt 0 -and ($moved / $points) -lt ${plan.stillFraction}) { $stillRun++ } else { $stillRun = 0 }
        if ($stillRun -ge ${plan.stillConfirmations}) { $endedBecause = 'still'; break }
      }
      $previousSample = $frame.sample
    }`
    : '';
  return `$clock = [System.Diagnostics.Stopwatch]::StartNew()
  $frames = @()
  $previous = $null
  $previousSample = $null
  $stillRun = 0
  ...
  foreach ($path in $paths) {
    ...
    $frame = Save-Frame $rect $path ([bool]$cursor) ([bool]$sample)
    $frames += @{
      offsetMs = $offset
...
try {
  ...
  $sample = ${plan.untilStill === true ? '$true' : '$false'}
  ${burstLoop(plan)}
...
while ($true) {
  ...
  try {
    ...
    if ($request.kind -eq 'capture') {
      ${RECT_RESOLVER}
      $frame = Save-Frame $rect $request.path ([bool]$request.cursor) $false
    ...
    elseif ($request.kind -eq 'burst') {
      ...
      $cursor = [bool]$request.cursor
      $sample = [bool]$request.untilStill
      ${burstLoop({ untilStill: true, stillFraction: '$request.stillFraction', stillConfirmations: '$request.stillConfirmations' })}
      Emit @{ id = $request.id; ok = $true; width = $rect.Width; height = $rect.Height; endedBecause = $endedBecause; frames = $frames }
    }
...
/**
 * Run one script in a process of its own, and decode its result.
 *
 * The script goes to a file rather than onto the command line. It used to
 * travel as `-EncodedCommand`, which was fine while the script was small and
 * stopped being fine as it grew: base64 of UTF-16 doubles the byte count, and a
 * script carrying the C# shim, the rectangle resolver and the burst loop reached
 * ~27KB of command line — close enough to Windows' 32KB limit that a capture
 * failed to spawn at all with `ENAMETOOLONG`. A file has no such limit, and it
 * is also the only way PowerShell will read non-ASCII correctly, so it is
 * written with a BOM.
 *
 * The name carries a hash of the contents, so repeated calls with the same
 * output path reuse one file instead of littering the temporary directory, and
 * a changed script gets its own file rather than overwriting one another
...
async function runScript(script, options = {}) {
  ...
  // The script file is written before the host is looked for, and the order is
  // deliberate: what PowerShell will be handed is the part of this function
  // that can be asserted on a machine that has no PowerShell — a macOS or Linux
  // checkout — and a case does assert it there. Checking the host first made
  // that assertion unreachable off Windows, which is how the suite came to have
  // a case that failed on the platform it was written for rather than testing
  // the platform it is about.
  //
  // Best effort on the shim cache, and deliberately not awaited into the
  // failure path: if the directory cannot be made, the script compiles the shim
  // in memory exactly as it did before there was a cache.
  await mkdir(dirname(shimCachePath()), { recursive: true }).catch(() => {});
  const scriptPath = runScriptPath(script);
  ...
  const result = await run(powershell, ['-NoLogo', '-NoProfile', '-NonInteractive', '-ExecutionPolicy', 'Bypass', '-File', scriptPath], {
...
async function captureBurst(plan, options) {
  ...
  const parsed = await runRequest(
    {
      ...engineRequestFor(plan),
      kind: 'burst',
      paths,
      intervalMs: plan.intervalMs ?? 0,
      untilStill: plan.untilStill === true,
      stillFraction: plan.stillFraction,
      stillConfirmations: plan.stillConfirmations,
    },
    () => burstScript(plan, paths),
    options,
  );
  const reported = Array.isArray(parsed.frames) ? parsed.frames : [];
  // A burst that ends early because the picture settled returns fewer frames
  // than it asked for, and that is the point of asking it to watch. Anything
  // else about the count is still a defect.
  if (reported.length > paths.length) {
    throw new CaptureError(
      `the Windows capture engine returned ${reported.length} frames for the ${paths.length} asked for`,
      { kind: 'capture-failed' },
    );
  }
  if (reported.length < paths.length && parsed.endedBecause !== 'still') {
    throw new CaptureError(
  ...
  return {
    frames,
    spacingMs,
    ...(parsed.endedBecause === undefined || parsed.endedBecause === 'frames'
      ? {}
      : { endedBecause: parsed.endedBecause }),
  };
}
