...
/**
 * Where each display sits on the desktop, as the running engine reports it.
 *
 * This is data macOS otherwise does not give up: `system_profiler` describes a
 * display's size and whether it is the main one, and says nothing about where it
 * is, so a plugin reading only that cannot tell a caller where a second screen
 * begins. ScreenCaptureKit knows, because it has to in order to capture at all,
 * and the engine's readiness line already carries it.
 *
 * The coordinates are points in the same space a `region` request uses — the
 * engine subtracts them itself when converting a request into the framework's
 * per-display space.
 *
 * @returns one entry per display, main first, or `undefined` when no engine is
 *   running: it is a fact the helper holds and the plugin does not, so it is
 *   absent rather than guessed when the helper is not there.
 */
export function engineDisplayOrigins() {
  if (!engineIsWarm()) return undefined;
  const listed = engine.info?.displays;
  if (!Array.isArray(listed)) return undefined;
  return listed.map((display, position) => ({
    index: position + 1,
    x: display.x,
    y: display.y,
    width: display.width,
