...
export function run(command, args, options = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    // `windowsHide` is what keeps a console application spawned from a
    // console-less host — the harness, restarted by a detached helper — from
    // having Windows allocate it a visible console window. That window would
    // sit on the user's desktop for the length of every capture, and closing it
    // would kill the capture with it. It is ignored on POSIX, so it costs
    // nothing to state on every platform.
