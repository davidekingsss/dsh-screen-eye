 ...
 *
 * The detail is appended only when it says something the message does not
 * already carry. Both engines build their errors out of the system's own words
 * — `new CaptureError(detail, { detail })` — so the two fields are the same
 * string for every failure the engine itself reported, and appending it
 * produced `display 99 does not exist: display 99 does not exist`. A message
 * that stutters reads like a malfunction in the plugin rather than a fact about
 * the screen, which is the one thing an error must not do here.
 *
...
export function describeCaptureFailure(error) {
  if (error instanceof CaptureError) {
    ...
    return new Error(`${error.message}: ${error.detail}`);
  }
  return error;
}
