  ...
  return defineTool({
    ...
    parameters: {
      ...
      wait_for_change: {
        type: 'boolean',
        description:
          'Start the burst when the picture changes, instead of when this call arrives. Use it for a transition that runs ONCE — a hover, a panel opening, a page load: say you are watching, ask the user to trigger it, and the frames begin at the moment it starts. Without it a burst takes its first frame before the animation happens, and a 300ms transition is over before the second one lands. The call returns only once it has seen the change, so a still screen means it waits.',
      },
      wait_timeout_ms: {
        type: 'integer',
        description:
          `How long wait_for_change waits for something to move, in milliseconds. Defaults to ${DEFAULT_WAIT_TIMEOUT_MS}. When nothing changes in that window the call fails and says so, rather than returning frames of a screen that never moved.`,
      },
    },
    output: {
      schema: {
