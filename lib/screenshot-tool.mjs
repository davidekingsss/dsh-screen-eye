...
export function screenshotTool(ctx, readSettings, log = noopLogger) {
  ...
  return defineTool({
    ...
    async execute(args, exec) {
      ...
      const screenOrigin = screenOriginFor(plan);
 ...
 *
 * Not reported for `display`: macOS's `system_profiler` lists a display's size
 * but not where it sits, so a second screen's origin is not knowable here.
 * Guessing would be worse than staying silent, because the value would look
 * authoritative and be wrong.
 *
 * @param plan - the validated capture request.
 * @returns `{ x, y }`, or undefined when the origin is not known.
 */
export function screenOriginFor(plan) {
  if (plan.mode === 'region') {
