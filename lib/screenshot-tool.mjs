  ...
  return defineTool({
    ...
    parameters: {
      ...
      duration_ms: {
        type: 'integer',
        description:
          'How much real time to cover, in milliseconds. Use this instead of frames and interval_ms when you know how long '
          + 'the thing you want to watch lasts — a few hundred milliseconds for a UI animation, a couple of seconds for a '
          + 'window opening — and the tool will fit as many frames as it can across that window. Mutually exclusive with '
          + 'frames and interval_ms.',
      },
      interval_ms: {
        type: 'integer',
        description:
          `Target gap between frames in milliseconds when frames is above 1. Defaults to ${DEFAULT_BURST_INTERVAL_MS}. `
          + 'It is a target, not a promise, and how low it can go depends on the area: a capture costs about 45ms plus '
          + 'encoding, measured at 155ms for a full screen but 56ms for a 1200x800 region. So to resolve a short '
          + 'animation — a few hundred milliseconds — capture the small region it happens in and lower this, rather '
          + 'than watching the whole screen; the reply reports the spacing actually achieved.',
      },
      path: {
        type: 'string',
        description:
    ...
    async execute(args, exec) {
      ...
      if (plan.mode === INVENTORY_MODE) {
        const displays = await listDisplays({ signal: exec.signal });
