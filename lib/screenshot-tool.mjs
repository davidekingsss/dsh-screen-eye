  ...
  return defineTool({
    ...
    parameters: {
      mode: {
        ...
        enum: CAPTURE_MODES,
        description:
          'What to capture. "screen" grabs every display as one image and is the default; "display" grabs the single display named by display; "region" grabs the rectangle named by region. "window" and "select" are interactive — they wait for the user to click a window or drag a rectangle, so use them only when the user asked to choose.',
      },
      region: {
        type: 'string',
        description:
          'Required by mode "region": the rectangle to capture as "x,y,w,h" in screen points from the top-left of the main display, e.g. "0,0,800,600".',
      },
      display: {
        type: 'integer',
        description: 'Display to capture for mode "display", 1-based (1 is the main display).',
      },
      include_cursor: {
        type: 'boolean',
    ...
    async execute(args, exec) {
      ...
      try {
        ...
        throw toToolError(error, settings.locale);
      }
 ...
 */
function toToolError(error, locale) {
