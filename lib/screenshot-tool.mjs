  ...
  return defineTool({
    ...
    parameters: {
      ...
      interval_ms: {
        ...
        description:
          ...
          + 'costs proportionally less. It is a target, not a promise, and how low it can go depends on the area: a '
          + `capture costs about 45ms plus encoding, measured at 155ms for a full screen but 56ms for a 1200x800 region, `
          + `so to resolve a short animation capture the small region it happens in rather than the whole screen. `
    ...
    async execute(args, exec) {
      ...
      if (plan.mode === INVENTORY_MODE) {
        const displays = await platformFor().listDisplays({ signal: exec.signal });
