...
import { DEFAULT_TIMEOUT_MS, resolveOutputPath } from './settings.mjs';

/** The longest a single call may declare, and so the largest `timeout_ms`. */
const MAX_CALL_TIMEOUT_MS = 900000;
...
const DESCRIPTION_FRAMES = 'Set frames above 1 to watch something move: the frames come back together in this one call, sampled at the interval you choose, which is how a process that changes over time can be seen. The interval is the setting that decides how much you spend and how finely the motion is read; the frame count is the ceiling. When the burst waits for a change, the picture decides where it ends. Recording a GIF is not an option here — the harness stores images single-frame, so an animated GIF would arrive as its first frame.';

/**
 * How to read a screen, in the words the model actually gets.
 *
 * This is the one place the plugin can say *how* to use it, and without it the
 * only way to learn the shape of the job is to fail at it. The tool described
 * what it captures and when to reach for it, but a caller that has never seen
 * this plugin had no way to know that reading a screen is done in passes: the
 * measured behaviour was a first full-screen capture, then the region it
 * actually needed, reached by trial rather than by knowing.
 *
 * Written as an observation about how this work goes rather than as an order.
 * The honest position is that the caller can see the situation and this text
 * cannot — a page-load animation, a video, or a whole-desktop question are all
 * cases where the overview *is* the answer. The cost of choosing wrong is a
 * wasted call rather than a wrong picture, so the judgement stays with the
 * model; this only makes sure it knows there is a choice to make.
 */
const DESCRIPTION_PASSES = [
  'Reading a screen usually takes two passes rather than one.',
  'An overview — the whole screen, or one display — shows where things are and which part of the picture is worth a closer look.',
  'A region then shows that part at one image pixel per screen pixel, which is how text, small icons and exact positions become readable.',
  'So something hard to make out in the overview is usually legible in a region covering it, while something that fills the screen, moves, or is the subject of the whole question is what the overview is for.',
  // The overview is for deciding, the region is for measuring. Stated because
  // the difference is easy to miss and the cost is paid in repeated calls.
  'The division of labour is worth keeping in mind: the overview tells you where to look, and the region tells you what is there.',
].join(' ');
 ...
 */
export function screenshotTool(ctx, readSettings, log = noopLogger) {
  ...
  return defineTool({
    ...
    parameters: {
      mode: {
        ...
        enum: CAPTURE_MODES,
        description:
          'What to capture. "screen" captures the main display and "display" captures the single display named by display — both are overviews of a whole screen, and both come back reduced to fit an image budget.'
          + ' "region" captures exactly the rectangle named by region at one image pixel per screen pixel, up to the budget, which is what to reach for when you need to read something or measure where it is.'
          + ' "displays" captures nothing and lists the connected displays with the index display expects, so you can find out whether a second monitor exists.'
      ...
      region: {
        ...
        description:
          ...
          // The workflow, stated where it is actionable rather than in the
          // description, which already says which pass is for what. This is the
          // sentence that would have saved two wasted calls in a measured
          // session: an overview is a picture to judge by eye, and treating it
          // as a ruler is how a region lands on the row above the one intended.
          + 'A rectangle can be estimated from an overview and then corrected: the reply names the screen position of the region it took, so a second attempt can be aimed rather than guessed — and aiming twice beats measuring once. An overview that was reduced is not a ruler: one pixel of it covers several pixels of the screen, so a coordinate read off it lands near the mark rather than on it.',
      },
      display: {
        type: 'integer',
        description:
      ...
      frames: {
        type: 'integer',
        description:
          `How many captures to take, all returned in this call, and the number you are spending. Defaults to 1 — one `
          + `capture. Frames, interval_ms and duration_ms describe one burst and any two of them determine the third; `
          + `giving all three is refused. At most ${MAX_BURST_FRAMES} in one call, which is the provider's own limit on `
          + `images per request rather than a policy here: past it the harness replaces the extra images with a `
          + `placeholder, so they would be captured and then not seen. `
          + `What a frame costs you: one image, and at most ${MAX_TOKENS_PER_FRAME} vision tokens (measured about 380 for `
          + `a full-screen capture) — so ten frames is about 4k tokens and sixty about 23k. That price is the whole `
          + `reason to think about the count, and it is yours to spend: this tool will not decide it for you. `
          + `If the burst waits for a change first, the ending is the picture's to decide and frames is only a ceiling, `
          + `so a generous count costs nothing on a motion that stops. Not available with the interactive modes.`,
      },
      duration_ms: {
        type: 'integer',
        description:
          'How much real time to cover, in milliseconds. This is a convenience for when the window is the thing you '
          + 'know — give it alone and it is sampled at the ordinary frame count, give it with frames to choose how '
          + 'coarsely the window is sampled, or with interval_ms to sample it as densely as that allows. It is NOT how '
          + 'you control the size of a burst: the interval is. Prefer interval_ms when what you care about is how '
          + 'closely the motion is followed.',
      },
      interval_ms: {
        type: 'integer',
        description:
          `Gap between frames in milliseconds — how finely the motion is sampled, and the parameter to think in when `
          + `you are deciding how much of a picture to buy. Twice the interval is half the frames for the same span and `
          + `half the tokens; a quarter of it is four times both, and only worth it when the motion is faster than the `
          + `coarser sample can read. ${briefing.timing} `
      ...
      timeout_ms: {
        type: 'integer',
        description:
          `How long this whole call may take, in milliseconds — the wait for a change included, with the capture `
          + `getting whatever the wait did not spend. Defaults to ${DEFAULT_TIMEOUT_MS}. Raise it when you are watching `
          + `something slow: a one-second interval over a minute-long process needs more than the default leaves room `
          + `for once a wait is paid for. Nothing else in this tool is bounded by a fixed duration — the burst stops `
          + `when its own ending arrives — so this is the ceiling to raise, not a window to fill.`,
      },
      path: {
        type: 'string',
        description:
    ...
    output: {
      schema: {
        ...
        properties: {
          ...
          // Where the returned picture's top-left corner sits on the screen, in
          // the same coordinate space `region` takes. Reported only when it is
          // known for certain — see `screenOriginFor` — because a wrong origin
          // is worse than an absent one: it would turn every coordinate the
          // model derives from the picture into a plausible mistake.
          screenOrigin: {
            type: 'object',
            additionalProperties: false,
            properties: {
    ...
    // can actually kill — see lib/exec.mjs, which does exactly that. The
    // declaration is the ceiling a call may ask for, not the budget a call
    // gets: the harness enforces this one and throws the work away when it
    // fires, so a caller asking for more than this is refused with the reason
    // rather than allowed to run into a deadline that discards the frames. The
    // budget actually applied is `timeout_ms`, defaulting to the plugin's.
    timeoutMs: MAX_CALL_TIMEOUT_MS,
    // Captures read the screen and write distinct files, so they may overlap.
    isConcurrencySafe: () => true,
    async execute(args, exec) {
      // Read once per call, so a setting edited while the host is running is
      // what the next capture uses — the settings page writes to a document
      // this plugin is a layer over, not to a copy taken at mount.
      ...
      // The budget for this call: the plugin's default, or whatever the caller
      // needs for the job in front of it. A burst watching a slow process has
      // no way to know the default will not do, so the number it would
      // otherwise be stuck with is the caller's to set — up to the ceiling the
      // tool declares, because past that the harness's own deadline fires and
      // discards work that was already done.
      const timeoutMs = args.timeout_ms ?? settings.timeoutMs;
      if (!Number.isInteger(timeoutMs) || timeoutMs < 1000 || timeoutMs > MAX_CALL_TIMEOUT_MS) {
        throw new CaptureError(
          `timeout_ms must be a whole number of milliseconds between 1000 and ${MAX_CALL_TIMEOUT_MS}; `
          + `received ${JSON.stringify(args.timeout_ms)}`,
        );
      }
      ...
      // Housekeeping, before this call's files exist rather than after each of
      // them does. Retention keeps the newest N captures, and a burst is no
      // longer capped at ten, so pruning per frame would delete the beginning
      // of the very burst being taken — the paths in the reply would name files
      // that had already been removed. Once per call, the directory is bounded
      // just as tightly and the frames this call is about are untouched.
      //
      ...
      // business walking it looking for files to delete. Failure here is never
      // fatal: housekeeping must not fail a capture that worked.
      if (settings.keepRecent > 0) {
        await pruneCaptures(settings.outputDir, settings.keepRecent, log, basename(outputPath));
      }
      ...
      // Computed from the plan rather than from the capture: it describes where
      // the picture was aimed, which is a property of the request, and it holds
      // for every frame of a burst because a burst keeps one rectangle.
      //
      // `display` is the one mode that needs the inventory to answer, because
      // where a screen sits is a fact only the platform knows — and on macOS
      // only its resident helper knows it. The lookup is skipped for the modes
      // that do not need it, so the ordinary path pays nothing for it.
      const displays = plan.mode === 'display'
        ? await platformFor().listDisplays({ signal: exec.signal }).catch(() => undefined)
        : undefined;
      const screenOrigin = screenOriginFor(plan, displays);
      ...
      const shared = {
        ...
        // Where this picture sits on the screen, so a coordinate measured
        // inside it can be turned into a screen coordinate by adding rather
        // than by undoing the downscale. Omitted when the origin is not
        // knowable — see `screenOriginFor`.
        ...(screenOrigin === undefined ? {} : { screenOrigin }),
...
/**
 * Where the picture a plan will produce sits on the screen, when that is known.
 *
 * This exists so the model can *add* rather than divide. A capture comes back
 * reduced to fit an image budget, and the reply says by how much — but the
 * arithmetic that follows from that ("multiply coordinates by 1.41") is the
 * step that actually went wrong in practice: measuring a feature on a reduced
 * picture and scaling it back up is error-prone, and a model that does it lands
 * near the thing rather than on it. Handing over the screen position of the
 * picture's own corner turns the same job into a single addition, which is both
 * easier and harder to get wrong.
 *
 * Reported only when it is certain:
 *
 * - `region` — the rectangle is the caller's own, so its origin is known
 *   exactly;
 * - `screen` — by definition the main display, whose top-left corner is the
 *   origin of the coordinate space `region` is expressed in, so it is (0, 0);
 * - `display` — where the platform's inventory reports an origin for that
 *   screen. macOS does not report one from `system_profiler`, but its resident
 *   capture helper does, so the field appears while the helper is running and is
 *   absent otherwise. It is never guessed: a wrong origin is worse than a
 *   missing one, because every coordinate derived from the picture would look
 *   authoritative while being off by a whole screen.
 *
 * @param plan - the validated capture request.
 * @param displays - the platform's inventory, when one is already in hand.
 * @returns `{ x, y }`, or undefined when the origin is not known.
 */
export function screenOriginFor(plan, displays) {
  if (plan.mode === 'region') {
    const [x, y] = plan.region.split(',').map(Number);
    return { x, y };
  }
  if (plan.mode === 'screen') return { x: 0, y: 0 };
  if (plan.mode === 'display' && Array.isArray(displays)) {
    const found = displays.find((display) => display.index === plan.display);
    if (Number.isInteger(found?.x) && Number.isInteger(found?.y)) return { x: found.x, y: found.y };
  }
  return undefined;
}
