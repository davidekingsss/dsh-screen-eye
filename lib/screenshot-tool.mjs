  ...
  return defineTool({
    ...
    parameters: {
      ...
      region: {
        type: 'string',
        description:
          'Required by mode "region": the rectangle to capture as "x,y,w,h" in screen points. The origin is the top-left of the main display, so a display positioned to its left or above takes negative coordinates, for example "-1920,0,800,600". Width and height must be positive. A rectangle can be estimated from an overview and then corrected: the reply names the screen position of the region it took, so a second attempt can be aimed rather than guessed.',
      },
      display: {
        type: 'integer',
        description:
