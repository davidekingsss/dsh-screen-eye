  ...
  return defineTool({
    ...
    parameters: {
      ...
      wait_for_change: {
        type: 'boolean',
        description:
          'Start the burst when the picture changes, instead of when this call arrives. Use it for a transition that runs ONCE — a hover, a panel opening, a page load: say you are watching, ask the user to trigger it, and the frames begin at the moment it starts. '
          + 'Aim it at a REGION that holds the animation and little else, which usually means capturing the component once first to find its rectangle. Watching the whole screen fails in both directions: a clock, a notification or another window starts the burst instead, and the sampling is spread so thin that a small animation may not register at all. '
          + 'A transition shorter than about 100ms can be over before a change is confirmed; for those, capture the region directly and compare the frames you get. The call returns only once it has seen the change, so a still screen means it waits — and if nothing moves within wait_timeout_ms it fails and says so.',
      },
      wait_timeout_ms: {
        type: 'integer',
        description:
          `How long wait_for_change waits for something to move, in milliseconds. Defaults to ${DEFAULT_WAIT_TIMEOUT_MS}. Size it for how long the user needs: it has to cover the moment between issuing this call and triggering the animation.`,
      },
    },
    output: {
      schema: {
