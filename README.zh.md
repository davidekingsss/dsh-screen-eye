...
- **算准目标**：给出用户在列表里要找的那个名字，以及需要手工添加时的完整路径；
...
**这里有三种状态，而且只有一种是「从未授权」**——我们在 macOS 26.6.2 上把三种都制造出来
实测过，因为它们的表现完全不同：
...
所以真实流程是**四步**，请按顺序走完，**中途不要反复重试**：

1. **留意系统弹窗**：「「运行 Deepseek Harness」想要录制此电脑的屏幕和音频。」若出现，
   点 **「打开系统设置」**——它会**自动**把这个 App 加进列表。
   若没出现（从终端启动、或曾被拒绝过），自己打开
   **系统设置 → 隐私与安全性 → 屏幕与系统音频录制**。
2. **在列表里找到那个条目**。它可能已经在了——就是上一步的弹窗加进去的。
   列表显示的是**文件名**而不是完整路径，所以要找的名字通常是 `node`；
   工具报告里会明确给出这个名字和对应的完整路径。
