...
- **连拍。** Windows 上单次截图稳态约 380ms，且几乎与面积无关，因为每次调用都要付
  一次 PowerShell 启动（其中 148ms 是它什么都不做时的启动成本）；它需要的 C# 垫片
  每台机器只编译一次，之后从缓存加载，这一项每次省 177ms。六帧连拍若拆成六次调用，
  会把一段 400ms 的动画采样成两秒半，所以 Windows 把整段连拍放进同一个引擎进程，
  计划里的间隔才成为可达的。
