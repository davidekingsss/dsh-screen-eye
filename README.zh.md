...
所有键都是可选的——而且它们全都可以在 harness 自己的设置页里改：**设置 → 插件 →
Screen Eye**，或者直接手改 `~/.dsh/settings.yaml` 里的 `screen-eye:` 一节。
两种方式都是**下一次调用即生效、不需要重启**，并且插件的挂载条目始终是"清空某个
字段后回落到的那一层"。三层关系、卡片的形态、以及它背后的两条平台约束见
...
- **只播一次的动画。** 组件过渡只播一次，而模型不可能在用户点击的那一瞬间发出调用
  ...
  区间内。macOS 有同样的选项，只是检测手段更慢——它没有可常驻的助手进程可问。
...
第一步是最关键的一条——macOS 的静止判定靠的是两张截图的哈希比对而不是采样像素，
它成立的前提是"画面未变时，剥掉描述性块后的字节完全相同"。
